...
class UserLoginSerializer(serializers.ModelSerializer):
    email = serializers.EmailField(max_length=255, min_length=4,style={'input_type':'email'},required=True)
    password = serializers.CharField(max_length=68, min_length=6,required=True,write_only=True,style={'input_type': 'password'})
    class Meta:
        model = User
        fields = ('email', 'password','username','tokens')
    ...
    def validate(self, attrs):
        ...
        if not user:
            raise AuthenticationFailed('Invalid username or password, try again with valid Credentials')
        if not user.is_active:
            raise AuthenticationFailed('Account is not activate , please contac with admin')
        return {
            'email': user.email,
            'username': user.username,
            'tokens': user.tokens
